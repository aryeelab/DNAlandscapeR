...
As HiC data, depending on the resolution, can vastly exceed the file sizes of other tracks, we compress the typical output of processed HiC data into a list of matrices where each element of the list is a chromosome and the matrix contains the interaction scores for the particular chromosome. The coordinates of each chromosome are index in the row and column names. This list of matrices is saved/loaded in a .rds file for fast data access. We note that **DNAlandscapeR** utilizes only the lower triangle of the data matrix when plotting interaction scores. Below is a sample R script that was used to take output from [HiC-Pro](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-015-0831-x) and convert the GM12878 HiC data at 1Mb resolution to an .rds object that **DNAlandscapeR** can plot. 
```
library(readr)
...
out.file <- paste(out.pre, ".rds", sep = "")
...
HiC.list <- lapply(core_chrom, function(chr){
...
names(HiC.list) <- core_chrom
saveRDS(HiC.list, file = out.file)
...
All data can be downloaded from our Amazon S3 instance using `wget`. For example, accessing the .rds of the GM12878 ChIA-PET data at 1Mb resolution could be achieved by running the following command--
```
wget http://s3.amazonaws.com/data/human/hic/GM12878_100000.rds
```
